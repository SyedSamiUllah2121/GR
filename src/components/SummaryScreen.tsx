...
export const SummaryScreen: React.FC<SummaryScreenProps> = ({ inspectionId }) => {
  ...
  if (!inspection || !model) {
    return (
      <div className="p-8 max-w-xl mx-auto text-center">
        <h2 className="text-xl font-bold text-[#242217]">Summary not found</h2>
  ...
  return (
    <div className="p-4 md:p-8 max-w-4xl mx-auto w-full">
      ...
      {/* Heading */}
      <div className="mb-6 pb-4 border-b border-[#DEDACB] flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          <p className="text-[10px] font-bold uppercase tracking-widest text-[#635E4F]">
            Inspection summary
          </p>
          <h1 className="text-2xl md:text-3xl font-bold tracking-tight text-[#242217] mt-1">
            {inspection.branchName}
          </h1>
          <div className="flex flex-wrap items-center gap-4 mt-2 text-xs text-[#635E4F]">
            ...
            <span className="capitalize font-semibold text-[#2F5233]">{inspection.status}</span>
      ...
      {(model.missingCount > 0 || model.scoreMismatch) && (
        <div className="mb-6 p-4 rounded-md bg-[#F3ECD8] border border-[#8A6318]/30 flex items-start gap-3">
      ...
      <section id="summary-at-a-glance" className="mb-6">
        <h2 className="text-[10px] font-bold uppercase tracking-wider text-[#635E4F] mb-2.5">
          At a glance
        </h2>
        ...
        {unanswered.length > 0 && (
          <p className="mt-2.5 text-xs font-semibold text-[#8A6318]">
      ...
      <section id="summary-priorities" className="mb-6">
        ...
        <div className="bg-white border border-[#DEDACB] rounded-md p-4 shadow-xs flex flex-wrap gap-2">
      ...
      <section id="summary-issues" className="mb-6">
        ...
        {model.issues.length === 0 ? (
          <div className="p-6 bg-[#E7EEE4]/60 border border-[#2F5233]/20 rounded-md text-center">
            ...
            <p className="text-xs text-[#2F5233]/80 mt-1">Nothing was flagged on this visit.</p>
          </div>
        ) : (
          <div className="space-y-5">
            {SEVERITY_ORDER.filter((s) => model.severityCounts[s] > 0).map((severity) => (
              <div key={severity}>
                ...
                <div className="border border-[#DEDACB] rounded-md divide-y divide-[#DEDACB] overflow-hidden bg-white">
                  {model.issues
                      ...
                      return (
                        <div
                          ...
                          className={`p-3.5 flex items-start gap-3 ${
                            severity === 'critical' ? 'bg-[#F4E4DF]/40' : ''
                          }`}
                        >
                          <span className="text-xs font-bold text-[#635E4F] w-8 shrink-0 pt-0.5 tabular-nums">
                            {row?.number}
                          </span>
                          <div className="flex-1 min-w-0">
                            <p className="text-sm font-semibold text-[#242217]">{item.text}</p>
                            {section && (
                              <p className="text-[10px] font-bold uppercase tracking-wider text-[#635E4F] mt-0.5">
                            ...
                            {priority.repeatCount > 0 && (
                              <p className="text-xs font-semibold text-[#8A6318] mt-1">
                          ...
                          {answer.photo && (
                            <img
                              ...
                              className="w-14 h-14 object-cover rounded-md border border-[#DEDACB] bg-white shrink-0"
      ...
      {model.repeats.length > 0 && (
        <section id="summary-repeats" className="mb-6">
          ...
          <div className="bg-[#F3ECD8] border border-[#8A6318]/30 rounded-md p-4 shadow-xs">
            <div className="flex items-start gap-3">
              <History className="w-5 h-5 text-[#8A6318] shrink-0 mt-0.5" />
        ...
        <div className="bg-white border border-[#DEDACB] rounded-md divide-y divide-[#DEDACB] overflow-hidden shadow-xs">
          {byCategory.map((cat) => (
            <div key={`${cat.listKey}::${cat.key}`} className="px-4 py-2.5 flex items-center gap-4">
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold text-[#242217] truncate">{cat.title}</p>
                <p className="text-[10px] font-bold uppercase tracking-wider text-[#635E4F]">
                  {cat.listLabel}
                </p>
              </div>
              <div className="w-24 sm:w-40 h-1.5 bg-[#DEDACB] rounded-full overflow-hidden shrink-0">
                <div
                  className={`h-full ${
                    cat.rate === 100
                      ? 'bg-[#2F5233]'
              ...
              <span className="text-xs font-bold text-[#242217] tabular-nums w-20 text-right shrink-0">
        ...
        <section id="summary-evidence" className="mb-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
            {model.photos.map((row) => (
              <figure
                key={row.item.id}
                className="bg-white border border-[#DEDACB] rounded-md overflow-hidden shadow-xs"
              >
                <img
                  src={row.answer!.photo as string}
                  alt={`Evidence for item ${row.number}`}
                  className="w-full h-28 object-cover bg-[#F5F3EC]"
                  referrerPolicy="no-referrer"
                />
                <figcaption className="p-2.5">
                  ...
                  <p className="text-xs text-[#242217] mt-1.5 leading-snug">
      ...
      <section id="summary-signoff">
        ...
        <div className="bg-white border border-[#DEDACB] rounded-md p-4 shadow-xs flex flex-wrap items-center gap-6">
          {inspection.signature ? (
            <div className="w-56 h-20 border border-[#DEDACB] bg-[#F9F8F4] rounded-md flex items-center justify-center p-2">
              <img
            ...
            <div className="w-56 h-20 border border-dashed border-[#DEDACB] bg-[#F9F8F4] rounded-md flex items-center justify-center text-xs text-[#635E4F] italic">
              Not signed yet
            </div>
          )}
          <div className="text-xs text-[#635E4F] space-y-1">
            <p>
    ...
    tone === 'good'
      ...
      : tone === 'bad'
        ...
        : tone === 'warn'
          ...
          : 'text-[#242217]';
  return (
    <div className="bg-white border border-[#DEDACB] rounded-md p-3.5 shadow-xs">
